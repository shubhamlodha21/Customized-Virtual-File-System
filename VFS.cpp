...
#define READ 1
#define WRITE 2 //  permission as (1+2)3 For Both Read & Write
...
typedef struct inode  //94 bytes gets allocated for this block
{
	...
	int InodeNumber;     // Number of inodes
 	int FileSize;        // 1024 byte
	int FileActualSize;  // This gets allocated when we write into the file
	int FileType;        // File is regular(1) OR special(2)
	char *Buffer;        // 
	int LinkCount;       // No of places where file get copied
 	int ReferenceCount;  //
	int permission;      // Read(1) Write(2) or Execute
...
typedef struct filetable
{
	int readoffset;
	int writeoffset;
	int count;//Always one
	int mode;// Opened mode R W RW
...
} // Pointer Which Points To File Table
 UFDT;
...
UFDT UFDTArr[MAXINODE];    // Created Array of structure
...
void man(char *name)
{
    ...
    { 
       printf("Description : To take backup of file in hardisk\n"); 
       printf("Usage : backup\n"); 
    } 
    else 
    { 
...
PINODE Get_Inode(char *name)
{
	...
	while(temp!=NULL)//Travels till Last of LL
	{
		if(strcmp(name,temp->FileName)==0)//If both name are same 
		{
			break;
		}
		temp=temp->next;
...
void CreateDILB()
{
	...
	while(i<=MAXINODE)
	{
		newn=(PINODE)malloc(sizeof(INODE));//creates node

		newn->LinkCount=newn->ReferenceCount=0;
		newn->FileType=newn->FileSize=0;
		newn->Buffer=NULL;
		newn->InodeNumber=i;//Only InodeNumber=1 others are set 0
...
int CreateFile(char *name,int permission)
{
	...
	if((name==NULL)||(permission==0)||(permission>3))//Filter to Create file
	{
		return -1;
	}

	if(SUPERBLOCKobj.FreeInode==0)//If Inode not Avaliable
	{
		return -2;
	}
	...
	(SUPERBLOCKobj.FreeInode)--;//After Creating file freeinode

	while(temp!=NULL)
	{
		if(temp->FileType==0)//Find Unused Inode
		{
			break;
		}
		temp=temp->next;
	...
	while(i<50)
	{
	    if(UFDTArr[i].ptrfiletable==NULL)//Search Unused iNode
		{
			break;
		}
		i++;
	...
	UFDTArr[i].ptrfiletable=(PFILETABLE)malloc(sizeof(FILETABLE));//Create File Table

	if(UFDTArr[i].ptrfiletable==NULL)//IF Malloc Fails
	{
		return -4;
	}
	...
	strcpy(UFDTArr[i].ptrfiletable->ptrinode->FileName,name);//FileName Copied67890-
	...
	memset(UFDTArr[i].ptrfiletable->ptrinode->Buffer,0,MAXFILESIZE);//Free Dynamically created memory

	return i;//UFDT Index return
} 
...
void backup()
{
	...
	while(temp!=NULL)
	{
		if(temp->FileType!=0)
		{
			fd=creat(temp->FileName,0777);//System Call Internal calls

			
			write(fd,temp->Buffer,temp->FileActualSize);//Write in file
		}
		temp=temp->next;
	}
}	
...
int main()
{
   ...
   while(1)
   {
   	...
   	if(count==1)
   	{
   		...
   		{
   			backup();
   			printf("Backup of files Successfully..!");
   			continue;
   		}
